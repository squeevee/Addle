...
void RasterSurface::initialize(QImage::Format format, QRect area, InitFlags flags)
{
    _buffer = QImage(area.size(), format);
    _area = area;
}
...
QImage RasterSurface::copy(QRect area, QPoint* offset) const
{
    QRect copyRect = _area.intersected(area);
...
void RasterSurface::allocate(QRect area)
{
