...
private:
    ...
    void clearTransientCache() const
    {
    ...
    struct CacheEntry
    {
        ...
        std::size_t childrenCount = SIZE_MAX;
...
void ObservedTreeItemModelHelperBase<ItemModel, Tree>::setTree(const Tree& tree)
{
    ...
    if constexpr (boost::mp11::mp_valid<aux_datatree::_datatree_observer_t, const Tree&>::value)
    {
        _state = aux_datatree::ObservedTreeState(aux_datatree::tree_observer(tree));
    }
    else
    {
...
QModelIndex ObservedTreeItemModelHelperBase<ItemModel, Tree>::index_impl(int row, const QModelIndex& parent) const
{
    ...
    DataTreeNodeAddress parentAddress = parentEntry ? parentEntry->address : DataTreeNodeAddress();
    DataTreeNodeAddress address = DataTreeNodeAddressBuilder(parentAddress) << row;
    ...
    else
    {
        ...
        entry->transientIt = _transientCache.insert(_transientCache.begin(), entry);
        _locationCache[address] = entry;
        
        pruneTransientCache();
    }
    
    return static_cast<const ItemModel*>(this)->createIndex(row, 0, entry);
}
...
QModelIndex ObservedTreeItemModelHelperBase<ItemModel, Tree>::parent_impl(const QModelIndex& index) const
{
    ...
    {
        row = parentAddress.lastIndex();
        
        auto find = _locationCache.find(parentAddress);
        if (find != _locationCache.end())
        {
            parentEntry = (*find).second;
            
            if (parentEntry->transientIt != _transientCache.end())
            {
                _transientCache.splice(
                        _transientCache.begin(), 
                        _transientCache, 
        ...
        {   
            parentEntry = new CacheEntry { parentAddress };
            parentEntry->transientIt = _transientCache.insert(_transientCache.begin(), parentEntry);
            _locationCache[parentAddress] = parentEntry;
            
            pruneTransientCache();
        }
    }
    
    return static_cast<const ItemModel*>(this)->createIndex(row, 0, parentEntry);
}
...
bool ObservedTreeItemModelHelperBase<ItemModel, Tree>::hasChildren_impl(const QModelIndex& parent) const
{
    ...
    {
        std::size_t cachedCount = parentEntry ? parentEntry->childrenCount : _cachedRootChildCount;
    ...
    auto mappedAddress = _state.mapToCurrent(parentAddress, &isDeleted);
    
    if (Q_LIKELY(_state.isCurrent() || !isDeleted))
    {
    ...
    {
        for (QModelIndex index : noDetach( static_cast<const ItemModel*>(this)->persistentIndexList() ))
        {
            const DataTreeNodeAddress& address = static_cast<const CacheEntry*>(index.internalPointer())->address;
...
std::size_t ObservedTreeItemModelHelperBase<ItemModel, Tree>::rowCount_impl(const QModelIndex& parent) const
{
    ...
    std::size_t& cachedResult = parentEntry ? parentEntry->childrenCount : _cachedRootChildCount;
    ...
    else
    {
        ...
        // is unusual but not strictly unavoidable, and can be caused by an
        // awkwardly timed UI event.
        // 
        ...
        // find the smallest value that won't cause glitches.
        //
        // If the pending DataTreeNodeEvent that deleted this node is processed within 
        // the current cycle of the UI thread's event loop, this should not 
        // result in any noticeable side-effects.
...
ObservedTreeItemModelHelperBase<ItemModel, Tree>::getNode(const QModelIndex& index) const
{
    auto lock = _state.lockObserverForRead();
    ...
    DataTreeNodeAddress address = entry ? entry->address : DataTreeNodeAddress();
    
    bool isDeleted = false;
    auto mappedAddress = _state.mapToCurrent(address, &isDeleted);
        
    if (!_state.isCurrent() && isDeleted)
    {
        return {};
    }
    ...
    if (!entry) 
        ...
        return (entry->directHandle = aux_datatree::node_lookup_address(_root, address));
}

template<class ItemModel, class Tree>
void ObservedTreeItemModelHelperBase<ItemModel, Tree>::onNodesChanged(DataTreeNodeEvent event)
{
    auto lock = _state.lockObserverForRead();
    ...
    std::unordered_set<CacheEntry*> diff(_persistentCache);
    
    const auto fromPersistentIndices = static_cast<ItemModel*>(this)->persistentIndexList();
    ...
    for (QModelIndex index : fromPersistentIndices)
    {
        ...
        _locationCache.erase(entry->address);
            
        bool deleted = false;
        entry->address = event.mapForward(
                std::move(entry->address),
                SIZE_MAX,
                &deleted
            );
        ...
        {
            assert( diff.erase(entry) );
        }
        
        toPersistentIndices.append(static_cast<ItemModel*>(this)->createIndex(
    ...
    // clean up entries in _persistentCache that are not (anymore) in the
    // persistent index list
    for (CacheEntry* entry : diff)
    {
        _locationCache.erase(entry->address);
        _persistentCache.erase(entry);
        delete entry;
    }
